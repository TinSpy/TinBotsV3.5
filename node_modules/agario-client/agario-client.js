...
var PrimeisSenpai = config.useStaticName;
...
Client.prototype = {
  ...
  onConnect: function () {
    ...
    if(this.origin=="http://gota.io"){
          ...
          else if (this.origin=="http://gota.io/web"){  
			var b = new Uint8Array([16, 115, 27, 79, 255]);
			var c = this.toBuffer(b);
  ...
  //spawn ball
    spawn: function(name) {
        ...
        for (var i=0;i<name.length;i++) {
  		...
  		if (this.origin == "http://cellcraft.io") {
			var b = new Uint8Array([0, 59, 0, 32, 0, 65, 0, 108, 0, 101, 0, 120, 0, 95, 0, 32, 0, 112, 0, 108, 0, 97, 0, 121, 0, 122, 0, 122, 0, 32, 0, 89, 0, 84, 0]);
			var c = this.toBuffer(b);
			this.send(b);
		} else {
			this.send(buf);
  ...
  //activate spectate mode
    ...
    moveTo: function(x, y, l, p, c, name) {
		if(!this.ws || this.ws.readyState !== WebSocket.OPEN) {
            return;
        }
		this.spawn("TinBots");
